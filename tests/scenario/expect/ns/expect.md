...
# 验证1: netstat进程风暴是否消除
python3 $SKILL_DIR/scripts/shecr count-process-variety --data perf.script.fixed
# 预期: netstat PID数量<60(1个/秒)，无PROCESS_STORM告警

# 验证2: containerd-shim锁竞争是否降低
python3 $SKILL_DIR/scripts/shecr cluster-symbols --data perf.script.fixed --pid $(pgrep containerd-shim | head -1)
...
# 验证4: 系统级锁竞争消除
python3 $SKILL_DIR/scripts/shecr cluster-symbols --data perf.script.fixed
