...
- [诊断流程反思](#诊断流程反思)
  - [领域知识被遗忘 (v0.7)](#领域知识被遗忘-v07)
  - [目标一致性 (v2.7)](#目标一致性-v27)
  - [审计独立性 (v2.32)](#审计独立性-v232)
  - [Live Document 机制 (v2.9)](#live-document-机制-v29)
- [方法论演进](#方法论演进)
- [工具设计教训](#工具设计教训)
- [输出系统演进](#输出系统演进)
- [文档体系演进](#文档体系演进)
...
早期设想中，审计（audit）被视为诊断流程的一部分，在 `finalize` 之前执行检查：
```
diagnose → audit check → finalize → export
              ↓ (发现问题)
         reopen / block
```
...
这种设计存在根本缺陷：
- **阻塞诊断节奏**：审计员发现问题会中断工程师的诊断流程
- **权责混淆**：审计员变成「 gatekeeper 」，而非质量反馈者
- **流程复杂**：audit 与 finalize 的状态交互难以设计

**决策对比**

| 维度 | 方案 A：审计阻塞 finalize | 方案 B：完全独立 (最终选择) |
|------|------------------------|--------------------------|
| 执行时机 | finalize 之前必须通过 audit | 诊断完成后任意时间 |
| 交互关系 | audit 可以 reopen / block | audit 与 finalize 无交互 |
| 审计结果用途 | 诊断继续的门槛 | 质量反馈和学习 |
| 工程师体验 | 被打断、等待审计 | 独立完成诊断 |

**最终设计**
```
诊断流程（工程师主导）:
  trace init → analysis → trace complete → finalize → export
                                                  ↑
审计流程（独立事后）:                               |
  trace audit（任何时候运行，结果不反馈到诊断流程）----┘
```

**经验教训**

- **独立性原则**：质量审计必须与诊断执行分离，避免干扰
- **非阻塞设计**：反馈用于学习改进，而非流程控制
- **单一职责**：`finalize` 标记诊断完成，`audit` 提供质量评估，两者互不依赖
