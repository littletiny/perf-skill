...
# SPEAR-perf-hunter v2.15 更新日志

## 更新概览

本次更新是**文档体系的大规模重组**，解决原有文档职责边界模糊、信息分散的问题。

**核心改进**:
1. **建立 5 层文档架构**，清晰划分职责边界
2. **新增 workflow-patterns.md**，集中 5 种典型分析模式
3. **重构 SKILL.md 为快速开始版本**，压缩 67%
4. **拆分 workflow.md** → workflow-core.md + workflow-patterns.md
5. **净化 tools.md**，纯命令参考，分析策略移至模式文档
...
| workflow.md 过大 | 514 行，包含 7 Phase + 典型模式 | 查找困难，信息过载 |
| tools.md 定位不清 | 既是命令参考，又包含分析要点 | 参考时分散注意力 |
| 缺乏场景化路径 | 按工具类型组织，非问题类型 | "sys 开销高怎么办？"需翻 3 个文档 |

### 1.2 新文档架构（5 层模型）

```
┌─ 第一层：入口引导层
│   └─ SKILL.md [重构，102 行] - 快速开始 + 场景速查
│
├─ 第二层：场景模式层 [新增]
│   └─ workflow-patterns.md - 5 种典型场景的完整分析路径
│
├─ 第三层：核心流程层
│   └─ workflow-core.md [拆分，~400 行] - 7 Phase 分析流程
│
├─ 第四层：参考层
│   └─ tools.md [净化，~330 行] - 纯命令参考
│
└─ 第五层：规则层
    ├─ heuristics.md - 启发式规则
    ├─ templates.md - 文档模板
...
| 操作 | 文件 | 说明 |
|------|------|------|
| **新增** | `references/workflow-patterns.md` | 典型分析模式专册（15737 行） |
| **重构** | `SKILL.md` | 311 行 → 102 行，快速开始版本 |
| **拆分** | `references/workflow-core.md` | 原 workflow.md 核心流程部分 |
| **重命名** | `references/workflow-legacy.md` | 原 workflow.md 保留备份 |
| **净化** | `references/tools.md` | 移除分析要点，纯命令参考 |

---

## 2. 新增 workflow-patterns.md
...
| tmp.md 概念 | 在文档中的体现 |
|------------|--------------|
| **V 型对称模型** | 决策树可视化 + 每个模式强调 Top-down 确认 + Bottom-up 溯源 |
| **三候选准则** | 每个模式包含"三候选假说示例"表格（代码/架构/环境维度） |
| **驱动力分析** | 每个模式开头明确三种驱动力（请求流量/系统资源/内部机制） |

### 2.3 新增模式 E: 高内核态分析

**场景**: `kernel% > 50%`

**解决原有空白**: 之前 sys 开销高的指导分散在 3 个文档的 7+ 个位置，缺乏集中式路径。

**分析路径**:
```bash
# Step 1: 确认影响范围
get-comm-top / check-cpu-bottleneck

# Step 2: 语义聚类定位内核活动类型
cluster-symbols --comm <xxx>  # 识别 SCHEDULER/IRQ_OFF/LOCK_CONTENTION/MEM_RECLAIM

# Step 3: 根据聚类结果选择溯源目标
find-callers --target schedule/<lock>/...
...
| 假说 | 机制 | 验证工具 |
|------|------|---------|
| E1: 内核锁竞争 | 多进程竞争内核全局锁 | `cluster-symbols` + `find-callers` |
| E2: 调度器活动频繁 | 进程/线程状态切换频繁 | `cluster-symbols` (EVENT_SCHEDULER) |
| E3: 系统调用风暴 | 业务逻辑大量 syscall | `find-callers --target __x64_sys_*` |
...
1. **三分钟开始**: 文档创建 + Live Document 初始化步骤前置
2. **典型场景速查**: 5 行表格直接映射 workflow-patterns.md 的 5 个模式
3. **文档分层架构**: 明确 5 层结构，引导用户按需深入
4. **保留关键信息**: 关键检查点表格、强制审计流程、典型陷阱
...
原 `workflow.md` (514 行) 拆分为两份文档：

| 新文档 | 内容 | 行数 |
|--------|------|------|
| `workflow-core.md` | 7 Phase 核心流程 + 关键检查点 + 审计流程 | ~400 行 |
| `workflow-patterns.md` | 5 种典型分析模式完整路径 | ~350 行 |
...
修改前:
  查看 SKILL.md → 不确定看哪里
  查看 workflow.md → 在 514 行中翻找
  可能遗漏关键信息

修改后:
  查看 SKILL.md "典型场景速查"
  → 找到"模式 E: 高内核态"
  → 点击直达 workflow-patterns.md 模式 E 完整路径
```
...
**移除**:
- 分析要点（移至 workflow-patterns.md 各模式）
- 使用场景描述（移至 workflow-patterns.md）
- 性能问题分类（移至 workflow-core.md）
...
**纯命令参考手册**，用户知道要执行什么命令时，快速查阅参数和输出格式。

分析策略和方法论引导至：
- 典型场景 → `workflow-patterns.md`
- 核心流程 → `workflow-core.md`
...
| 原来查 | 现在查 |
|--------|--------|
| SKILL.md 详细教程 | SKILL.md 快速开始 → workflow-patterns.md 具体模式 |
| workflow.md 典型模式 | workflow-patterns.md 完整模式 |
| workflow.md 7 Phase | workflow-core.md 核心流程 |
| tools.md 分析要点 | workflow-patterns.md 各模式开头 |

### 对于新用户

推荐阅读顺序：
1. **SKILL.md** - 3 分钟了解整体，识别自己的场景
2. **workflow-patterns.md** - 按场景执行完整分析路径
3. **tools.md** - 查询具体命令参数
4. **heuristics.md** - 深入理解方法论背后的规则
...
- [x] 文档体系 5 层架构清晰
- [x] SKILL.md 压缩至 102 行，保留核心信息
- [x] workflow-patterns.md 包含 5 种完整模式
- [x] 模式 E（高内核态）填补原有空白
- [x] tmp.md 核心概念（V 型/三候选/驱动力）已融合
- [x] tools.md 净化为纯命令参考
- [x] 所有文档交叉引用正确
- [x] 历史文档 workflow-legacy.md 保留备份
...
1. **模式扩展**: 根据实际案例新增更多典型模式（如内存问题、IO 问题）
2. **自动化**: 基于 workflow-patterns.md 的结构化内容，开发 `plan` 子命令
3. **案例库**: 为每种模式补充真实案例分析
