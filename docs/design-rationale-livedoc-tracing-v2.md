...
v1.0 设计需要**人工执行** `spear doc add` 和 `spear doc complete`：
...
# 必须人工执行！
spear doc add --id ISS-001 --desc "xxx" --risk "xxx"

# 分析完再人工标记完成
spear doc complete --id ISS-001 --result "xxx"
```
...
# 或者选择忽略
spear doc complete --id ISS-001 --result "wontfix: 优先级低"
```
...
```bash
# 查看 timeline
spear doc timeline [--format json]

# 查看 issues 状态  
spear doc issues [--status open|resolved|all]

# 标记 issue 完成（人工执行）
spear doc complete --id <issue_id> --result <result>

# 最终审计（检查是否还有 open issue）
spear doc finalize [--accept-risk <reason>]

# 导出报告
spear doc export [--format markdown|json]
```
...
[2] 10:01:00  cluster-symbols --comm netstat --data netstat_perf.data
    ...
    ℹ️  分析结果: LOCK_CONTENTION 38.36%, /proc/net/tcp 竞争
        (ISS-001 仍需人工确认: spear doc complete --id ISS-001 --result "...")
...
⚠️  ISS-001  netstat 高内核态 94.7%
    ...
    └─ 确认解决: spear doc complete --id ISS-001 --result "LOCK_CONTENTION 38.36%"
