...
| 决策 | 理由 |
|------|------|
| 独立 `shecr trace` 工具 | 不改造现有工具，最小侵入 |
...
# Step 2: 立即记录所有问题
$ shecr trace add --id ISS-001 --desc "netstat 高内核态 94.7%" --risk "进程风暴" --hint "cluster-symbols --comm netstat"
$ shecr trace add --id ISS-002 --desc "containerd-shim 高内核态 89.9%" --risk "可能比 netstat 更严重" --hint "cluster-symbols --comm containerd-shim"
...
$ shecr trace complete --id ISS-002 --result "LOCK_CONTENTION 79.84%"
