...
| 假设路径 | 机制评估 | 预期指纹 | 验证结果 | 状态 |
|---------|---------|---------|---------|------|
| **主动消耗**: 业务代码计算密集型 | **机制**: 用户态CPU占用主导<br>**副作用**: 内核态比例<30% | `get-comm-top`: user% > 60% | [实际user%和kernel%值] | [状态] |
| **被动压制**: Cgroup CPU限流 | **机制**: throttling触发调度延迟<br>**副作用**: analyze-core-distribution报告单核饱和 | `analyze-core-distribution`: 检测单核饱和 | [实际结果] | [状态] |
| **被动压制**: 内核瓶颈(调度/锁/IRQ) | **机制**: 内核态消耗高，调度器/锁竞争事件占比>10%<br>**副作用**: cluster-paths报告高比例Scheduling/Lock相关模式 | `cluster-paths`: [事件类型]>阈值 | [实际结果] | [状态] |
