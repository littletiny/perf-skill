...
| 版本 | 问题描述 | 关键证据引用 |
|------|----------|-------------|
| V1 | 用户反馈系统性能抖动 | `sys-audit`: 系统CPU [X]%, 内核态[Y]% |
| V2 | 发现单核饱和/负载不均衡/特定资源异常 | `analyze-core-distribution`: [瓶颈类型], [关键指标] |
| V3 | [精确定位具体问题，如进程风暴/锁竞争/系统调用过载等] | [相关工具]: [关键发现] |

---

## 二、竞争性假设追踪 (SHECR-H: Hypothesis)

**三候选准则**：同时保持**≥3条竞争性假设**，并行追踪直至证伪。
